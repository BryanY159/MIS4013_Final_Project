...
function editBrother($BID, $FN, $LN, $MC, $IY, $GY, $Major, $Major2, $Minor, $Minor2, $Status, $SID, $SP, $BP) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("UPDATE Brothers SET FirstName = ?, LastName = ?, MembershipClass = ?, InitiationYear = ?, GraduationYear = ?, 
						Major = ?, Major2 = ?, Minor = ?, Minor2 = ?, Status = ?, SectionID = ?, 
      						ServicePoints = ?, BrotherhoodPoints = ? WHERE BrotherID = ?");
